class CalculatorController {
  ...
  void _deleteDigit() {
    ...
    final length1 = displayOperations.length;
    if (length > 1) {
      displayOperations = displayOperations.substring(0, length1 - 1);
    } else {
      displayOperations = kEmpty;
    }
  }
  ...
  void _outputFormat() {
    ...
    displayOperations = _memories[kMemoryFirst].toString();
    displayOperations =
        displayOperations.endsWith('.0') ? result.replaceAll('.0', '') : result;
    displayOperations = displayOperations.replaceAll('.', kPoint);
  }
